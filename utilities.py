...
def get_data_as_train_test_split(test_size=.2, random_state=42):
    ...
    def sanitize_data(row):
        # removes any rows for which data is missing
        if all([row[0], row[1], row[2], row[3], row[4], row[5]]):
    ...
    star_features = []
    star_classes = []
    data_file_path = Path('stardata/Stars.csv')
    import csv
    with open(data_file_path) as csvfile:
        ...
        for row in reader:
            ...
            if data:
                # add the input features to the features list
                star_features.append(data)
                # add the class to the class list
                star_classes.append(int(row[6]))
